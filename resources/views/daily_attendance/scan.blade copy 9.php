...
<x-app-layout>
            ...
            <div class="col-md-6">
                ...
                <div class="shadow card border-bottom-primary">
                    ...
                    <div class="text-center card-body bg-light">
                        ...
                        <div id="reader" style="width: 100%; border: 2px solid #ddd; border-radius:8px; min-height: 300px; background: #000; overflow:hidden;"></div>
    ...
    <script>
        $(document).ready(function() {
            ...
            let isProcessing = false; // Mencegah scan ganda saat AJAX jalan
            ...
            function onScanSuccess(decodedText, decodedResult) {
                ...
                let targetUrl = (mode === 'daily') ? "{{ route('daily.store') }}" : "{{ route('izin.check') }}";
            ...
            // Update Instruksi & Warna Border saat ganti mode
            $('input[name="scan_mode"]').change(function() {
                if(this.value === 'permit') {
                    $('#reader').css('border-color', '#ffc107');
                    $('#scan-instruction').text('Mode Izin: Scan QR untuk verifikasi izin keluar.');
                } else {
                    $('#reader').css('border-color', '#3b82f6');
                    $('#scan-instruction').text('Mode Absensi: Scan saat datang & pulang.');
                }
            });
