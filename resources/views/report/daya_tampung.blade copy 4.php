...
<x-app-layout>
    @php
        ...
        try {
            // 1. Ambil Map Jurusan dari Database untuk lookup data tambahan (Program Keahlian, Ketua, dll)
            $majorsMap = \App\Models\Major::all()->keyBy('code');

            // 2. Mengambil seluruh kelas XII langsung (Logika yang Anda konfirmasi berhasil)
            ...
            $groupedKelas = $kelasXII->groupBy(function($k) {
                ...
                // Jika format nama "XII TITL 1", TITL ada di index ke-1
                return count($parts) > 1 ? strtoupper($parts[1]) : 'UMUM';
            });

            // 4. Menyusun format laporan dengan data Major dari DB
            foreach($groupedKelas as $code => $grup) {
                $jmlSiswa = $grup->sum(function($k) { return $k->students ? $k->students->count() : 0; });
                $jmlRombel = $grup->count();

                // Cari data tambahan dari tabel Majors berdasarkan Kode (TITL, dll)
                ...
                <div class="card shadow">
                    ...
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center no-print">
                        ...
                        <button onclick="printLaporan()" class="btn btn-primary btn-sm shadow-sm">
                            <i class="fas fa-print me-1"></i> Cetak Laporan Resmi
                        </button>
                    </div>

                    <div class="card-body bg-light border-bottom no-print text-muted py-2 text-center">
                        <small><i class="fas fa-info-circle me-1"></i> Masukkan estimasi angka pada kolom input manual. Total akan terhitung otomatis.</small>
                    </div>
                                    ...
                                    <tr>
                                        <td colspan="8" class="text-center py-5">
                                            ...
                                            <div class="small text-muted mt-2">
                                                Pastikan nama kelas Anda mengandung kata <strong>"XII"</strong> atau <strong>"12"</strong> <br>
                                                Contoh: XII TITL 1 atau 12 TITL 1.
                                            </div>
                                        </td>
                                    </tr>
                                    @endforelse
