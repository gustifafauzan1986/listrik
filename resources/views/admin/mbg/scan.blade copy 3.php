<x-app-layout>
                ...
                <div class="shadow-sm card">
                    ...
                    <div class="p-0 card-body">
                        <ul class="list-group list-group-flush" id="recent-list">
                            {{-- LOGIKA PHP UNTUK MENAMPILKAN DATA SAAT LOAD PAGE --}}
                            @php
                            ...
                            @forelse($recents as $row)
                                <li class="list-group-item d-flex justify-content-between align-items-center animate__animated animate__fadeInLeft">
                                    <div>
                                        ...
                                        <small class="text-muted">{{ \Carbon\Carbon::parse($row->check_in_time)->format('H:i:s') }} • {{ $row->student->nis ?? '-' }}</small>
                                    </div>
    ...
    <script>
        ...
        // Cek dukungan kamera dan minta izin
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices.getUserMedia({ video: true })
                .then(stream => { evidenceVideo.srcObject = stream; })
                .catch(err => console.log("Kamera bukti tidak aktif (Opsional):", err));
        }
        ...
        function processManual() {
            ...
            if(!nis) {
                Swal.fire("Peringatan", "Mohon isi NIS terlebih dahulu.", "warning");
                return;
            }
            isProcessing = true;
        ...
        // --- 3. KIRIM DATA KE SERVER (UPDATED) ---
        function processAttendance(nis, method) {
            // Tampilkan Loading
            document.getElementById('loading-scan').classList.remove('d-none');

            // Ambil Snapshot Wajah sebagai Bukti (Try Catch agar tidak error jika kamera mati)
            ...
            try {
                if(evidenceVideo.srcObject && evidenceVideo.videoWidth > 0) {
                    ...
                    imageBase64 = canvas.toDataURL('image/jpeg', 0.7); // Kompres 70%
                }
            } catch(e) {
                console.log("Gagal ambil snapshot: " + e);
            }
            ...
            fetch("{{ route('admin.mbg.store') }}", {
                ...
                headers: {
                    ...
                    "Accept": "application/json" // Paksa server respon JSON
                },
                body: JSON.stringify({
                    nis: nis,
                    method: method,
                    image: imageBase64 // Kirim foto bukti (bisa null)
                })
            })
            .then(async response => {
                // Handle respon non-JSON (biasanya HTML error 500)
                const contentType = response.headers.get("content-type");
                if (contentType && contentType.indexOf("application/json") !== -1) {
                    return response.json();
                } else {
                    const text = await response.text();
                    console.error("Server Error Response:", text); // Log error HTML ke console
