...
<x-app-layout>
    ...
    <style>
        ...
        /* Efek transisi saat loading AJAX */
        #tahfiz-container {
    ...
    <div class="py-4 page-content">
        ...
        <div class="mb-4 bg-white border-0 shadow-sm card">
            <div class="p-4 card-body">
                <form id="filterForm" action="{{ route('tahfiz.index') }}" method="GET" class="p-3 mb-0 border rounded bg-light">
                    <div class="row g-2 align-items-center">

                        <div class="col-md-5">
                            <div class="input-group">
                                <span class="bg-white input-group-text"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" id="search_input" name="search" class="form-control border-start-0" placeholder="Ketik nama siswa..." value="{{ request('search') }}">
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            // Fungsi utama untuk memanggil data via AJAX
            function fetch_data(page = 1) {
                let search = $('#search_input').val();
                let kelas_id = $('#kelas_filter').val();

                // Berikan efek transparan saat sedang loading
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        // Trik ajaib: Ambil hanya isi div '#tahfiz-container' dari response
                        let newContent = $(response).find('#tahfiz-container').html();
                        $('#tahfiz-container').html(newContent).css('opacity', '1');
                    },
                    error: function() {
                        $('#tahfiz-container').css('opacity', '1');
                        Swal.fire('Ups!', 'Terjadi kesalahan saat memuat data', 'error');
                    }
                });
            }

            // 1. Fitur Pencarian Real-time (Otomatis filter saat ngetik)
            let timer;
            $('#search_input').on('keyup', function() {
                clearTimeout(timer);
                timer = setTimeout(function() {
                    fetch_data(1); // Kembali ke page 1 setiap kali mencari
                }, 500); // Tunggu 500ms setelah selesai mengetik agar tidak spam server
            });

            // 2. Filter Dropdown Kelas otomatis berubah
            $('#kelas_filter').on('change', function() {
                fetch_data(1);
            });

            // 3. Mencegah form reload jika user menekan tombol 'Enter'
            $('#filterForm').on('submit', function(e) {
                e.preventDefault();
                fetch_data(1);
            });

            // 4. Fitur Tombol Reset
            ...
            // 5. Pagination AJAX (Agar pindah halaman tidak loading keseluruhan)
            $(document).on('click', '.ajax-pagination .pagination a', function(e) {
                e.preventDefault();
                let page = $(this).attr('href').split('page=')[1];
                fetch_data(page);

                // Scroll kembali ke atas kotak data
                $('html, body').animate({
                    scrollTop: $("#tahfiz-container").offset().top - 100
                }, 500);
            });
        });

        // Sweet Alert Session (Tetap dipertahankan)
        @if(session('success'))
            Swal.fire({
                icon: 'success',
                title: 'Berhasil!',
